...
export class ConversationCleanupService {
  ...
  constructor(private readonly conversationRepo: WhatsappConversationRepository) {}
  ...
  async cleanupOldConversations(): Promise<void> {
    ...
    this.logger.log(`Limpeza de conversas: ${deleted} registros removidos (anteriores a ${cutoff.toISOString().slice(0, 10)})`);
  }
}
